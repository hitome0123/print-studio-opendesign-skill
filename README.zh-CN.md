...
如果 Windows / PowerShell 弹出 `Invoke-WebRequest` 安全提醒，建议先点“否”，不要继续用该命令下载。这个提示通常来自 PowerShell 5.1 的网页脚本解析机制，不代表本 Skill 本身有病毒。

更稳妥的安装方式见：

```text
WINDOWS_SAFE_INSTALL.zh-CN.md
```

非技术使用者优先使用 GitHub 页面里的：
...
| 文档 | 用途 |
| --- | --- |
| `客户使用手册.zh-CN.md` | 给客户看的完整使用教程：怎么准备图片、怎么一键运行、怎么看交付包 |
| `WINDOWS_SAFE_INSTALL.zh-CN.md` | Windows 安全下载/安装说明，避免 `Invoke-WebRequest` 弹窗造成误会 |
| `user-workflow/prompt-templates.zh-CN.md` | 客户可直接复制到 Codex / Claude Code 的常用话术模板 |
| `user-workflow/guided-conversation-flow.zh-CN.md` | 泛化引导流程：无论做月历卡、贺卡、明信片、书签、吊牌还是挂历，Codex 都按同一套步骤带使用者选择 |
...
| `user-workflow/model-prompt-pack.zh-CN.md` | 给 Gemini、OpenAI GPT Image、GPT-Image2、即梦客户端使用的空白白底/氛围背景提示词包 |
...
1. 使用者先拖入图片文件夹，或发送图片文件夹路径。
2. Codex 先理解图片风格、颜色、主体位置、细节密度和系列统一性。
3. Codex 推荐一个默认方案，并给出 2–3 个替代选择，每个选择都说明理由和风险。
4. 使用者只需要一次回答一个小问题，例如选产品、选材质、选 A/B/C 版式。
5. Codex 先生成材质选择页和 A/B/C 版式候选页，不直接批量输出。
6. 使用者选中一版后，Codex 锁版，再批量生成预览、印刷文件、白底图、氛围图、4K 单张下载图。
7. 打开 `交付说明.html`，从一个入口检查和下载所有交付文件。
8. 用 QC 报告判断是否能交付。

每次在 Codex / Claude Code 里加载这个 Skill，它都会先用对话方式引导使用者：使用者可以直接拖拽图片文件夹或发送图片路径，再用自然语言说“想做月历卡/贺卡/明信片”。Codex 会自动调用 Skill 完成图片理解、规格推荐、材质预览、版式候选、锁版和交付包生成。使用者不需要一开始编辑 JSON 或复制命令。
...
它会从图片文件夹开始，一次性跑完：图片理解、材质预览、A/B/C 版式候选、自动锁版、完整交付包。适合第一次演示或给不想复制多条命令的使用者。
...
它会一步步询问图片文件夹、项目名称、产品类型、系列张数、材质和年份，并生成可直接运行的配置文件。
...
- `交付说明.html`：交付包首页，集中进入预览图、印刷文件、展示图、4K 下载、质检报告
- `交付说明.md`：纯文本交付说明
- `preview/`：材质与尺寸预览页，如果一键流程或预览流程已经跑过，会一起进入交付包
- `screen/`：预览版
- `print/`：300dpi 印刷版，含出血与裁切 guide
- `commerce/`：电商展示版、白底图、氛围图、系列总览图
- `download_4k/`：每张图单独保存的 4K 长边下载版，含 `index.html` 下载页
- `provider_previews/`：首轮可选的即梦 / Gemini / GPT-Image2 空白白底/氛围背景对比
...
```bash
python scripts/run_project_flow.py --images <图片文件夹> --theme <项目名>
```

如果想手动控制每一步，再使用下面的分步流程：
...
- `generate_layout_candidates.py`：先出 A/B/C 三个候选版式，并生成 `layout_candidates.json` 和 `layout_candidates/index.html` 可视化选择页。
...
V1 采用本地优先方式：配置、素材、输出都在本机文件系统里，不需要上传到外部服务。只有空白白底/氛围背景等可选 AI 生图环节可能调用配置中的图像模型后端，成品卡面不应交给模型改图。
...
| `provider_previews/` | 首轮模型比较 | 即梦 / Gemini / GPT-Image2 对比空白白底/氛围背景，重点看占位区是否完整无遮挡 |
...
- **生图模型只做背景气氛**：不要让模型直接处理成品卡面；白底图、氛围图建议由模型生成空白背景，再由本地合成叠加真实卡面。
