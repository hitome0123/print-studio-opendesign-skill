...
如果客户不想看 JSON，先看这 4 份操作文档：

| 文档 | 用途 |
| --- | --- |
| `customer-workflow/customer-order-form.zh-CN.md` | 客户接单配置表，销售/客服照填 |
| `customer-workflow/customization-options.zh-CN.md` | 可自定义项清单，说明哪些能改、哪些不建议乱改 |
| `customer-workflow/conversation-examples.zh-CN.md` | Codex / Claude Code 对话示例 |
| `customer-workflow/customer-acceptance-sop.zh-CN.md` | 客户验收 SOP，30 分钟跑通示例 |

推荐客户流程：

1. 填 `customer-order-form.zh-CN.md`。
2. 复制里面的提示词给 Codex 或 Claude Code。
3. 先生成材质选择页。
4. 再生成客户预览和印刷文件。
5. 用 QC 报告判断是否能交付。
