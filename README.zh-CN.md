...
Print Studio OpenDesign 是一个可安装到 Codex 的印刷品系列设计打样 Skill。它把“导入一组插画/图片 → 印刷厂选择产品规格 → AI 在规则内排版 → 输出预览、印刷文件、白底图、氛围图、QC 报告、交付 ZIP”的流程固化成可重复运行的工作台。
