...
如果无论点“是”还是“否”都会继续弹出，说明当前安装器正在循环调用 `Invoke-WebRequest`。这时不要继续点，也不要继续等它重试。

请直接：

1. 关闭弹窗。
2. 关闭当前安装窗口 / PowerShell 窗口。
3. 不再使用这个一键安装 / skill-installer 下载流程。
4. 改用下面的浏览器 ZIP 下载或 Git 克隆。
...
如果客户说点“是/否”都会继续弹，可以这样说：

> 这不是你操作错了，是当前安装器在循环调用 PowerShell 下载命令。请关闭弹窗和安装窗口，不要继续点。我们改用浏览器从 GitHub 下载 ZIP，解压后从本地文件夹安装。
